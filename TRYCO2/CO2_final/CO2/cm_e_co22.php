    ...
    <body>
      ...
      // 篩選員工
        if ((empty($start_date_display) && empty($end_date_display)) && !empty($filter_employee)) {
            ...
            // 修改查詢以包括員工名稱
            $chartQuery = "SELECT YEAR(em_co2.eCO2_date) AS year, 
                                MONTH(em_co2.eCO2_date) AS month, 
                                SUM(em_co2.eCO2_carbon) AS total_carbon, 
                                employee.em_name 
                        FROM em_co2
                        INNER JOIN employee ON em_co2.em_id = employee.em_id
            ...
            $employee_name = ''; // 初始化員工名稱

            if (!empty($chartResults)) {
                foreach ($chartResults as $row) {
                    // 獲取員工名稱
                    $employee_name = $row['em_name']; // 假設此查詢返回的結果只有一位員工
                    // 拼接月份和年份
            ...
            echo "<script>
                ...
                var layout = {
                    title: '員工 " . $employee_name . " 在 " . $currentYear . " 年度的碳排量',
                    xaxis: {
                        title: '月份',
                        type: 'category',
                        gridcolor: '#67776d' // 设置x轴网格线的颜色
       ...
       // 如果篩選日期、篩選員工
        if ((!empty($start_date_display) && !empty($end_date_display)) && !empty($filter_employee)) {
            ...
            // 修改查詢以包括員工名稱
            $chartQuery = "SELECT em_co2.eCO2_date, em_co2.eCO2_commute, SUM(em_co2.eCO2_carbon) AS total_carbon, employee.em_name 
                        FROM em_co2
                        INNER JOIN employee ON em_co2.em_id = employee.em_id

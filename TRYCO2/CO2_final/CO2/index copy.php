...
<body class="text-center justify-content-md-center">
    <!-- 導入導覽列 -->
...
if (isset($_SESSION['em_name'])) {
    $em_name = $_SESSION['em_name'];
} else {
    die("使用者名稱未設定，請先登入。");
}

// SQL 查詢，從 em_co2_transformed 抓取資料
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    // 將資料整理到以年份和月份分組的陣列中
    if (!isset($carbonData[$year])) {
        $carbonData[$year] = [];
    }
    $carbonData[$year][$month] = $total_carbon;
}

$jsonData = json_encode($carbonData); // 輸出 JSON 格式資料
// 回傳格式如下(參考)
// {
...
?>
    ...
    <script>
        ...
        function fetchAttendance() {
            ...
            fetch(`fetch_attendance copy.php?start_date=${startDate}&end_date=${endDate}`)
